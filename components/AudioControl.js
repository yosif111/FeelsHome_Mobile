...
export default class AudioControl extends Component {
  ...
  progressThread = null;
    ...
    this.setState({
      ...
      queue,
      currentTrack:
        status.track != "undefined" && status.track != null ? status.track : "",
      currentAlbum:
        status.album != "undefined" && status.album != null ? status.album : "",
      currentArtist:
        status.artist != "undefined" && status.artist != null
          ? status.artist
          : "",
      volume:
        status.volume != "undefined" && status.volume != null
          ? status.volume
          : 0,
      playerState:
        status.state != "undefined" && status.state != null
          ? status.state
          : "stopped",
      index:
        status.index != "undefined" && status.index != null ? status.index : 0,
      progress:
        status.progress != "undefined" && status.progress != null
          ? status.progress
          : 0,
      currentTrackLength:
        queue.length > 0 && status.index != null
          ? parseInt(queue[status.index].track_length / 1000)
          : 0,
      image:
        status.image != "undefined" && status.image != null
          ? { uri: "http:" + status.image }
          : DEFAULT_IMAGE
    });
    if (status.index == null || status.image == null) {
      let status = await api.getAllStatus();
      this.setState({
        index:
          status.index != "undefined" && status.index != null
            ? status.index
            : 0,
        image:
          status.image != "undefined" && status.image != null
